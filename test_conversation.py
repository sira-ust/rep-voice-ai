...
def _old_run(script: list[str], reply_window: float = 60.0) -> str | None:
    signed = el_get("/convai/conversation/get-signed-url?agent_id=" + urllib.parse.quote(AGENT))
    ws = WebSocketClient(signed["signed_url"], timeout=70)
    ws.send(json.dumps({
        "type": "conversation_initiation_client_data",
        "conversation_config_override": {"conversation": {"text_only": True}},
    }))
    ...
    conversation_id = None
    turn = -1          # -1 until the opener arrives
    quiet_since = time.time()

    while turn < len(script):
        try:
            raw = ws.recv()
        except (RuntimeError, socket.timeout):
            break
        ...
        if kind == "ping":
            ws.send(json.dumps({"type": "pong",
                                "event_id": evt.get("ping_event", {}).get("event_id")}))
            if time.time() - quiet_since > reply_window:
                print("      (no further reply -- moving on)")
                turn += 1
                if turn < len(script):
                    print("\n  you   : %s" % script[turn])
                    ws.send(json.dumps({"type": "user_message", "text": script[turn]}))
                    quiet_since = time.time()
            continue
        if kind == "conversation_initiation_metadata":
            conversation_id = evt["conversation_initiation_metadata_event"]["conversation_id"]
        ...
        if kind == "agent_response":
            ...
            if turn < 0:
                ...
                turn = 0
                print("\n  you   : %s" % script[0])
                ws.send(json.dumps({"type": "user_message", "text": script[0]}))
                quiet_since = time.time()
                continue
            ...
            # Pre-tool speech is short and ends with no substance; wait for the
            # real answer before advancing to the next scripted turn.
            if text and len(text) < 60 and text.rstrip().endswith(("...", ".", "!")) \
                    and any(w in text.lower() for w in
                            ("let me", "checking", "searching", "i will", "one moment")):
                continue
